...
export default expressjwt({
  ...
  async isRevoked(_req: Request, token) {
    try {
      ...
      const refreshToken = await refreshTokenRepo.findByRefreshIdAndUserId(
      ...
      logger.error(`Error while retrieving the refresh token : ${message}`, {
        refreshTokenId: (token?.payload as RefreshTokenPayload).refreshTokenId,
      });
    }
