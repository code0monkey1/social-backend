...
describe("DELETE /users/:userId", () => {
  ...
  describe("happy path", () => {
    it("should return json response with status code 200", async () => {
      await api.delete(BASE_URL + `/${userId}`).expect("Content-Type", /json/);
    });
    ...
    it("should delete associated user refreshTokens from the database", async () => {
      ...
      const tokensBefore = (await refreshTokenRepository.findAll()) as any;
  ...
  describe("un-happy path", () => {
    it("should return 401 when auth not provided", async () => {
      ...
      await api.delete(BASE_URL + `/${createdUser._id.toString()}`).expect(401);
    });
